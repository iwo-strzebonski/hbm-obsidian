---
tags:
  - location
  - planet
  - dwarf
  - khazad-morul
  - house-morul
aliases:
  - Khazad-Morul
  - Dom Morul
status: kanoniczna
---
| **Zdjęcie**                      | <img src="khazad-morul.jpg" width="200" alt="khazad-morul.jpg"> |
| -------------------------------- | ------------------------- |
| **Rodzaj**                       | *Planeta*                 |
| **Nazwa oficjalna**              | *Khazad-Morul*            |
| **Status**                       | *Zamieszkała, zamknięta*  |
| **Szacowana liczba mieszkańców** | *ok. 80 mln (krasnoludy)* |
| **Znane frakcje**                | Dom Morul, gildie rzemieślnicze |

## Opis

Khazad-Morul to ojczysty świat krasnoludów, położony w głębi galaktyki, znany z monumentalnych podziemnych miast, kopalń i zaawansowanej magii runicznej. Planeta jest niemal całkowicie zamknięta dla obcych – wejście możliwe tylko za zgodą Domu Morul. Główne miasta to rozległe kompleksy pod powierzchnią, połączone siecią tuneli i magicznych portali.

Krasnoludy słyną z mistrzostwa w rzemiośle, metalurgii i magii runicznej. Ich społeczeństwo jest silnie zhierarchizowane, a tradycja i honor odgrywają kluczową rolę. Khazad-Morul jest źródłem wielu legendarnych artefaktów i broni, a także miejscem narodzin najpotężniejszych runomagów.

## Powiązania fabularne

- Krasnoludy z Khazad-Morul są głównymi twórcami artefaktów runicznych w galaktyce.
- Dom Morul odgrywa kluczową rolę w polityce międzygwiezdnej, choć rzadko opuszcza planetę.
- Planeta jest celem ekspedycji poszukiwaczy artefaktów i dyplomatów, ale większość prób kończy się niepowodzeniem.

## Wskazówki dla MG

> [!gm]+ Tylko dla MG
> Khazad-Morul to miejsce pełne tajemnic, politycznych intryg i starożytnych sekretów. Możesz wykorzystać planetę jako źródło unikalnych artefaktów, arenę konfliktów gildii lub miejsce, gdzie bohaterowie muszą zdobyć zaufanie krasnoludów, by uzyskać dostęp do zakazanej wiedzy lub technologii.
