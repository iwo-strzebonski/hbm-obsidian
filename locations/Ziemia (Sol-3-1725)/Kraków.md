---
tags:
  - lokacja
  - ziemia
  - miasto
  - kraków
aliases:
  - Kraków
status: kanoniczna
---
| **Zdjęcie**                      | ![[krakow.jpg|200]] |
| -------------------------------- | ------------------------- |
| **Rodzaj**                       | *Miasto*                  |
| **Planeta**                      | *Ziemia (Sol-3-1725)*     |
| **Status**                       | *Aktywne, magiczne centrum* |
| **Liczba mieszkańców**           | *ok. 800 tys.*            |
| **Znane frakcje**                | Wschodnioeuropejska Szkoła Magii, Inkwizycja |

## Opis

Kraków to historyczne miasto na południu Polski, obecnie jedno z najważniejszych centrów magicznych Ziemi. Na jego obrzeżach, w subwymiarze, znajduje się Wschodnioeuropejska Szkoła Magii. Miasto jest miejscem spotkań dyplomatycznych, areną konfliktów magicznych i centrum badań nad nowymi dziedzinami magii.

Kraków łączy tradycję z nowoczesnością – gotyckie zabytki sąsiadują z magicznymi laboratoriami i siedzibami frakcji. Miasto jest kluczowym punktem na mapie Wielkiej Trzynastki i miejscem, gdzie krzyżują się losy wielu bohaterów.

## Powiązania fabularne

- Brama do subwymiaru WeSM znajduje się na południowych obrzeżach miasta.
- Kraków jest siedzibą wielu magicznych organizacji i miejscem audytów Inkwizycji.
- Miasto regularnie gości delegacje z innych planet i szkół magii.

## Wskazówki dla MG

> [!gm]+ Tylko dla MG
> Kraków to idealne miejsce na rozpoczęcie kampanii, polityczne intrygi, śledztwa Inkwizycji lub spotkania z przedstawicielami innych światów. Możesz wykorzystać miasto jako tło dla konfliktów frakcyjnych, eksperymentów magicznych lub tajnych negocjacji.
